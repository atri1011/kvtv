...
- **设置开启**：默认关闭，需先在普通设置页开启高级模式
- **首页入口**：开启后首页会显示高级入口，点击即可进入独立的高级视频专区
...
**特点：**
- 当前浏览器需先在普通设置页开启高级模式，首页才会显示入口并允许访问 `/premium`
- 开启高级模式后，访问 `/premium` 页面时仍需输入此专用密码
...
| `PREMIUM_PASSWORD` | 高级内容独立密码；在本地开启高级模式后，访问 `/premium` 时需输入 | - |
